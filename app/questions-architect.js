...
const ARCHITECT_Q = [
  ...
  },
  {
    ...
    a: [
      "Add a clear instruction telling the agent to always verify the customer's identity before any refund",
  ...
  },
  {
    ...
    a: [
      ...
      "The coordinator can no longer route all communication through itself, which breaks the hub-and-spoke structure",
    ],
    c: 1,
  ...
  },
  {
    ...
    a: [
      "It isn't weaker — the session already holds the whole change and the reasoning behind it, so it is the best-informed reviewer available",
      ...
      "A single claude -p invocation can't be called twice in one CI job, so the review step silently reuses the first run's output",
      "Nothing is weaker as long as the review step adds --bare, since a clean environment strips whatever bias the first run introduced",
    ],
    c: 1,
  ...
  // D3 additions
  {
    ...
    a: [
      ...
      "Remove the false-positive concern from the prompt entirely and rely on the reviewers to filter the results by hand",
    ],
    c: 0,
  ...
  },
  {
    ...
    a: [
      "The retry will succeed once the validation error tells the model exactly which field to look for",
      "The retry won't help — the information isn't in the source document at all, no amount of prompting will produce it",
      "The retry will succeed if you also raise the temperature so the model infers a plausible value from nearby fields",
      "The retry will succeed once the call switches to tool use, since a strict schema forces every field to be filled",
    ],
    c: 1,
  ...
  },
  {
    ...
    a: [
      "The synchronous API, since it is the only path that can guarantee schema-compliant output on every document",
      ...
      "The Message Batches API, since it guarantees a response within minutes even at large volumes and costs less per document",
      "The synchronous API run overnight in a loop, since the batch API cannot carry a JSON schema and the output structure would be lost",
    ],
    c: 1,
  ...
  },
  {
    ...
    a: [
      ...
      "The generating session has too little context window left by the time it reviews, so it cannot work through the whole diff carefully",
      "Self-review is disabled by default and needs a flag before the model will critique its own output",
      "The model samples more randomly on a second pass, so its judgment on material it just produced is unreliable",
    ],
    c: 0,
  ...
  // D4 additions
  {
    ...
    a: [
      "Return a normal success response with an empty result, so a malformed request cannot break the agent's loop mid-task",
      "Set the isError flag and include structured metadata such as errorCategory: \"validation\" plus a human-readable description",
      "Throw an unhandled exception so the connection drops and the client retries the call from scratch",
      "Return the plain string that the call failed, which tells the agent to stop without extra parsing",
    ],
    c: 1,
  ...
  },
  {
    ...
    a: [
      "Immediately propagate the raw timeout to the coordinator and stop, so that every retry decision in the system is owned in exactly one place",
      ...
      "Drop the failed step and report success, so one transient failure cannot cascade into aborting the whole run",
      "Halt the whole multi-agent run until a person intervenes, since a partial result is worse than no result",
    ],
    c: 1,
  ...
  },
  {
    ...
    a: [
      "Identically — both leave the agent without data, so treating them the same keeps the error path simple",
      ...
      "Zero matches should be retried once, since an empty result more often means a bad query than an empty dataset",
      "The tool should broaden the query internally until it finds at least one match, so the agent never sees an empty result",
    ],
    c: 1,
  ...
  },
  {
    ...
    a: [
      ...
      "Omitting tool_choice entirely, so the default behaviour applies for that turn",
    ],
    c: 1,
  ...
  },
  {
    ...
    a: [
      "Set tool_choice to auto on every turn and state the required ordering in the system prompt instead of enforcing it",
      ...
      "Set tool_choice to any on every turn, and rely on extract_metadata being listed first in the schema",
      "Remove the enrichment tools from the list entirely and add them back once metadata extraction is done",
    ],
    c: 1,
  ...
  },
  {
    ...
    a: [
      ...
      "Silently return an empty result so the workflow continues, letting the coordinator infer the gap later",
      "Terminate the whole multi-agent workflow as soon as any subagent fails, so no partial result is trusted",
      "Retry the identical query on a backoff until it succeeds, since a timeout is by definition transient",
    ],
    c: 0,
  ...
  },
  {
    ...
    a: [
      "Nothing — some drift is normal in a long session and the model recovers once you ask again",
      "Use /compact to reduce context usage, since the window has filled with verbose discovery output",
      "Switch to a different system prompt mid-session to reset how the model is reasoning",
      "Assume the session silently hit a hard token limit and restarted, and begin a new one",
    ],
    c: 1,
  ...
  },
  {
    ...
    a: [
      "There's no risk — a 97% aggregate already averages over every document type, so each type must be performing at roughly that level",
      ...
      "97% is too low for any automation, since an extraction pipeline has to be effectively exact before review can be reduced",
      "The number is fine, but it needs stratified random sampling of the low-confidence extractions before review can be cut",
    ],
    c: 1,
  ...
  },
  {
    ...
    a: [
      "Keep the figure from the more authoritative source and note the other in a footnote for completeness",
      "Average the two figures and report the mean, so the report carries one number readers can act on",
      "Annotate the conflict with source attribution and include each source's collection date, instead of picking one value",
      "Leave the statistic out of the report entirely, so a contested number cannot mislead a reader who only skims",
    ],
    c: 2,
  ...
  },
  {
    ...
    a: [
      "A stronger system-prompt rule stating that get_customer must always be called first, with the consequences spelled out for the model",
      "A few-shot example showing the correct call order, so the pattern is demonstrated rather than only described",
      ...
      "A retry loop that detects an out-of-order process_refund call and re-prompts the agent to start again",
    ],
    c: 2,
  ...
  },
  {
    ...
    a: [
      "Direct execution, since each file will reveal which integration path the code actually favours",
      "Plan mode, so the tradeoffs between the two integration paths are settled before any file is touched",
      "Plan mode with bypass permissions available, so exploration and the edits can proceed in one pass",
      "Direct execution one file at a time, so a wrong integration path stays contained to that file",
    ],
    c: 1,
  ...
  },
  {
    ...
    a: [
      "--resume and --json-schema, so the CI job continues where the last run stopped",
      "--verbose and --output-format markdown, so findings paste cleanly into a PR comment",
      "No special flags — Claude Code detects a non-interactive terminal and adapts on its own",
